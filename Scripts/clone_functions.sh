...
#	 snippets ("true") or not ("false"). (TODO: Går det att lösa snyggare?!)
# 2. Flag indicating whether empty snippets should be subtracted from the clones
#	 ("true") or not ("false"). (TODO: Går det att lösa snyggare?!)
################################################################################
analyzeClones() {
	frequencyFile=`./get_last_output.sh "cloneFrequency"`
	frequencyCol=$1
	subtract_empty_from_unique=$2
	subtract_empty_from_clones=$3
	...
	if [ "$subtract_empty_from_clones" = true ] ; then
		numClones=`echo "$numClones - $numEmpty" | bc`
	fi
